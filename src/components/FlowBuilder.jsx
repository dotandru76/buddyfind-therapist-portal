// src/components/FlowBuilder.jsx - V20.0 (Full Screen Canvas)
...
// --- רכיב חלון העריכה (Inspector) - צד ימין ---
const NodeInspector = ({ node, setNodes, setEdges, onClose }) => {
  ...
  return (
    <div style={{ 
        ...
        boxShadow: '-4px 0 20px rgba(0,0,0,0.05)', zIndex: 20, height: '100%'
    }}>
      <div style={{ padding: '20px', borderBottom: '1px solid #f1f5f9', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h4 style={{ fontWeight: '800', fontSize: '18px', color: '#1e293b' }}>עריכת שאלה</h4>
          <button onClick={onClose} style={{ border: 'none', background: 'none', fontSize: '20px', cursor: 'pointer', color: '#94a3b8' }}>&times;</button>
      </div>
      
      <div style={{ overflowY: 'auto', padding: '20px', flex: 1 }}>
          <div style={{ marginBottom: '20px' }}>
            <label style={{ display: 'block', fontSize: '13px', fontWeight: '600', color: '#64748b', marginBottom: '6px' }}>נוסח השאלה</label>
            <textarea value={label} onChange={(e) => { setLabel(e.target.value); updateNodeData('label', e.target.value); }} rows={3} style={{ width: '100%', border: '1px solid #cbd5e1', borderRadius: '8px', padding: '10px', fontSize: '14px', resize: 'none', outline: 'none' }} />
          </div>
          ...
          {questionType === 'slider' && (
              <div style={{ marginBottom: '20px', padding: '15px', background: '#f0fdf4', borderRadius: '8px', border: '1px solid #bbf7d0' }}>
                  ...
                  <div style={{ display: 'flex', gap: '10px' }}>
                      <div><span style={{ fontSize: '11px', color: '#15803d' }}>מינימום</span><input type="number" value={minVal} onChange={(e) => { setMinVal(e.target.value); updateNodeData('minVal', e.target.value); }} style={{ width: '100%', padding: '5px', borderRadius: '4px', border: '1px solid #ddd' }} /></div>
                      <div><span style={{ fontSize: '11px', color: '#15803d' }}>מקסימום</span><input type="number" value={maxVal} onChange={(e) => { setMaxVal(e.target.value); updateNodeData('maxVal', e.target.value); }} style={{ width: '100%', padding: '5px', borderRadius: '4px', border: '1px solid #ddd' }} /></div>
                  </div>
              </div>
          )}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {outputs.map((output, index) => (
                <div key={output.id || index} style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                  <div style={{ background: '#f8fafc', padding: '8px', borderRadius: '6px', flexGrow: 1, border: '1px solid #e2e8f0' }}>
                      <input type="text" value={output.label} onChange={(e) => updateOutputLabel(index, e.target.value)} style={{ width: '100%', border: 'none', background: 'transparent', fontSize: '13px', outline: 'none', fontWeight: '500' }} />
                  </div>
                  {questionType !== 'slider' && (
                      <button onClick={() => deleteOutput(index)} style={{ background: '#fee2e2', color: '#ef4444', border: 'none', borderRadius: '6px', width: '34px', height: '34px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '16px', transition: 'background 0.2s' }}>✕</button>
                  )}
                </div>
              ))}
            </div>
            {questionType !== 'slider' && (
                <button onClick={addOutput} style={{ width: '100%', marginTop: '15px', padding: '10px', background: '#eff6ff', color: '#3b82f6', border: '1px dashed #3b82f6', borderRadius: '8px', fontSize: '13px', fontWeight: '600', cursor: 'pointer', transition: 'background 0.2s' }}>+ הוסף תשובה</button>
            )}
          </div>
      </div>
    </div>
...
const FlowBuilderWrapper = ({ API_URL, onLogout }) => {
  ...
  return (
    <div className="bg-white shadow-sm h-full flex flex-col" style={{ height: 'calc(100vh - 100px)' }}>
      <div className="flex justify-between items-center px-6 py-3 bg-white border-b z-10 shadow-sm flex-shrink-0">
        <h3 className="text-2xl font-bold text-gray-800">עורך שאלון האבחון</h3>
        <div className="flex gap-3">
            <button onClick={addNode} className="px-4 py-2 bg-green-50 text-green-700 rounded-lg text-sm font-bold hover:bg-green-100 transition border border-green-200 shadow-sm">+ שאלה חדשה</button>
            <button onClick={onSave} className="px-6 py-2 bg-blue-600 text-white rounded-lg text-sm font-bold hover:bg-blue-700 transition shadow-md">שמור שינויים</button>
        </div>
      </div>

      <div className="flex flex-grow relative overflow-hidden">
        {/* אזור העבודה המרכזי (Canvas) */}
