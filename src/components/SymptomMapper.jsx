// src/components/SymptomMapper.jsx - V15.0 (Edge-to-Edge Studio Mode)
...
const SymptomMapperContent = ({ API_URL, onLogout }) => {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (res.ok) { alert(`✅ נשמר! (${mappings.length} קישורים)`); } else alert('❌ שגיאה');
        } catch (err) { alert('שגיאת תקשורת'); }
    };

    if (loading) return <LoadingSpinner />;

    // --- התיקון: פריסת מסך מלאה עם FIXED ---
    return (
        <div style={{ 
            position: 'fixed', top: '80px', left: 0, right: 0, bottom: 0, 
            zIndex: 40, backgroundColor: 'white', display: 'flex', flexDirection: 'column' 
        }}>
            <div className="flex justify-between items-center px-6 py-3 bg-white shadow-sm z-30 flex-shrink-0 border-b border-gray-200">
                <h3 className="text-2xl font-bold text-gray-800">מנוע ההתאמה (Studio Mode)</h3>
            ...
            <div className="flex flex-grow relative overflow-hidden">
                <div className="flex-grow relative h-full bg-slate-50" ref={reactFlowWrapper}>
                    <ReactFlow
                        ...
                        nodeTypes={nodeTypes} fitView minZoom={0.1} maxZoom={1.5}
